-- 여기는 STUDENT 사용자 화면
-- student 사용자는 현재 DBA 권한을 가지고 있따.
...
/*
RDBMS(Relation DataBase Mangement System)에서 데이터 취급하기
...
-- DROP TABLWE tbl_student; -- TABLE 제거하기
...
-- 사용할때 항상 주의를 해야한다.
-- 특히 칼럼의 크기(길이) 를 변경하거나, type를 변경할때는
-- 데이터 손상이 될 수 있기 때문에 매우 조심해야 한다.
ALTER TABLE tbl_student MODIFY (st_tbl VARCHAR2(20));

-- CRUD 실습
-- 1. 데이터 추가(CREATE)
-- st_num 칼럼은 PK로 선언되어 있기 때문에 데이터는 필수이며, 중복될수 없다.
-- st_num 칼럼은 PK로 선언되어 있기 때문에 NOT NUll, UNIQUE 성질을 갖는다
...
INSERT INTO tbl_student(st_num, st_name, st_dept, st_grade, st_tel)
VALUES('230003', '이몽룡', '행정학과', '2', '010-333-3333');
...
-- 3. Udate
-- Update를 수행할때는 내가 변경하는 데이터가 정말 변경하고자 하는 데이터인가?
-- 홍길동 학생의 전화번호가 010-111-1111 에서 010-111-1234로 변경되었다.
-- 데이터를 수정하라

-- 일반적인 Update 명령은 다음과 같다. 문법적으로는 아무런 문제가 없으나
-- 결과적으로 심각한 문제를 안고 있다.
-- 홍길동 이라는 이름이 동명이인이 있을 경우, 원하지 않는 데이터가 변경될 수 있다.
