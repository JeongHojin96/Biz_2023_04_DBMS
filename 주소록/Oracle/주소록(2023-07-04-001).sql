...
DDL 명령 : DBA 가 사용하는 명령들, 물리적 객체 생성, 제거, 변경을 수행하는 명령
    ...
    ALTER : 객체 변경, 사용중에 중요한 변경사항이 발생했을때
        구조변경을 수행하는 명령
        구조변경은 매우 신중하게 수행해야 한다.
*/

-- tbl_hobby table 의 이름변경
...
주소록의 ID 와 취미이름(hb_name)이 저장되어 있다.
hb_aid 칼럼은 주소 테이블과 연계되엉 ㅣㅆ지만
...
하지만 이명령은 이러한 원칙에 위배된다

이때 업데이트, 삭제를 수행하는 과정에서 이 테이블에는 수정이상, 삭제이상 현상이 발생할 수 있다.
또한 취미정보를 추가하는 과정에서 유사한 취미가 다른 형식으로 등록되는 경우가 있따.
...
AALTER TABLE tbl_hobby
RENAME TO tbl_addr_hobby;

SELECT * FROM tbl_addr_hobby;

-- 2. 취미 Relation 에서 취미이름 항목을 중복데이터 없이 추출하기
--      중복 없이 데이터를 모으기
-- hb_name 칼럼의 데이터를 같은 데이터끼리 그룹(묵어)지어 리스트를 만든다
...
-- 3. 제3정규화 데이터 import
-- 주의! 기존 제 2정규화 데이터를 엑셀로 보내서 제3정규화 데이터가 완료된 후
-- 다음을 진행할것
...
2, 3 정규화 된 Table 은 필연적으로 Table 간의 Join 이 되어야만
정보를 명확히 알 수 있다.
...
FROM tbl_addr_hobby, tbl_address;
 ...
 -- EQJOIN은 연계하는 Table 간에 참조무결성 제약조건이 완벽할떄는
 -- 아무런 문제가 없이 겨로가를 조회할 수 있따
...
-- LEFT JOIN
-- JOIN 을 실행하는 Table 간에 참조 무결성이 의심스럽거나 위배될지 모른다는
-- 전제하에 데이터를 확인하는 JOIN

-- FROM [table1] LEFT JOIN [table2]
-- 왼쪽(LEFT) table1 데이터는 무조건 SELECT 하고
-- 오른쪽(RIGHT) table2 데이터는 ON에서 설정한 조건에 맞는 것만 가져오기
-- 오른쪽의 table2 에 조건에 맞는 데이터가 있으면 보여주고
-- 없으면 null 표현
-- 1. SELECT 한 데이터의 누락이 발새
...
-- 주소취미 Relation 과 주소 Entity 간의 참조 무겷성 검사
...
-- 저장된 데이터를 모두 clear하고, 엑셀의 데이터로 다시 import 하자
...
-- view 의 생성
-- 물리적 Table 에 SQL 을 적용하여 작성하였떠니 다소 복잡한 SQL 이 완성되었다.
-- 이후 이 SQL을 사용할 일이 많을것 같다.
-- 그래서 이 SQL 을 view로 생성하였다.
...
REFERENCES tbl_address(a_id); -- tbl_address 의 a_id 칼럼과 연계
