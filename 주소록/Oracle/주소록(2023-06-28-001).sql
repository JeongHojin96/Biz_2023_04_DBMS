...
-- COUNT() 함수 : 테이블에 저ㅏㅇ된 데이터의 개수를 확인하는 함수
...
ANSI SQL : 모든 DBMS SW 에 거의 공통으로 구현된 명령 문법
        표준화된 문법으로 DBMS SW 가 달라도 사용할 수 있는 문법
...
-- 모든 칼럼(*) 로 조회를 하면, 실제 원하는 칼럼의 순서와 관계 없이
-- 데이터가 표현되기도 한다.
SELECT *
FROM tbl_address;
...
-- SELECT 명령에서 모든 칼럼(*)으로 조회하는 것 보다는 ALL Project으로 조회를
-- 하는 것이 , 조회 속도면에서 더 유리하다.
-- 코딩(java.. 등등)과 연동하여 사용할때는 가급적 ALL Project 를 사용하는 것이 좋다.
...
3. WHERE 절의 조건대로 데이터 selsection
4. select 절의 칼럼대로 데이터 projection
5. oreder by 절의 칼럼을 기준으로 정렬
...
특히 다중 조건을 부여할때,
조건문의 순서에 따라 SELECT 결과의 효율이 달라질 수 있따
...
SELECT *
FROM tbl_address
WHERE a_tel LIKE '090-33%' AND a_id >= 'A0001' AND a_id <= 'A0100';
