...
현재 INSERT 명령을 수행하면서 BUID에 해당하는 값을 지정(저장)하지 않았다.
*/
...
/*
TABLE 을 create 하면서 buid 와 buname 을 NOT NULL로 설정하였다.
데이터를 추가할때(INSERT)할때ㅔ 최소한 buid 값과 buname 값을
필수로 필요하다 라는 선언, 설정을 "제약조건" 설정 이라고 한다.
...
-- 매우 위험한 코드
-- Update, Delete를 수행할때는 PK가 아닌 칼럼을 기준으로 하지마라
-- 만약 PK가 아닌 칼럼을 기준으로 할때는 매우 신중하게 명령을 수행해야 한다.
UPDATE TBL_BUYER
...
1. bunme 칼럼의 데이터가 성춘향 인 리스트를 조회 한다.
2. 전화번호가 없는(null 인) 데이터의 buid 값을 확인한다 : 0003 임을 알 수 있다.
...
고객정보와 계좌정보를 연계하여 함꼐 볼수 있다면 좋겠네.

Table JOIN
2개(이상)의 Table을 서로 연계하여 하나의 리스트로 보기

tbl_acc와 tbl_buyer 테이블을 연계하여 하나의 리스트로 보여라
이때 tbl_acc의 acbuid 와 tbl_buyer buid 칼럼의 데이터를 비교하여
같은 데이터는 한 라인에 보여라
*/
SELECT * FROM TBL_ACC, TBL_BUYER
WHERE ACBUID = BUID;

-- JOIN을 하되 4개의 칼럼만 화면에 나타나도록 하고 싶다.
...
INSERT INTO tbl_buyer(buid, buname, butel)
VALUES ('0005', '김마노', '010-555-5555');
INSERT INTO tbl_buyer(buid, buname, butel)
VALUES ('0006', '장길산', '010-666-6666');

-- COMMIT 이 되기 전의 데이터를 취소하는 명령
ROLLBACK;

/* SQL Developer 와 Java 코드에서 DB를 서로 연동하여 처리하는 경우 발생하는 문제
SQD 에서 데이터를 INSERT, UPDATE, DELETE 를 수행하는 경우
    추가, 수정, 삭제된 정보는 아직 storage 에 반영되지 않고,
    메모리에 임시 보관(저장)된 상태이다
이 상태일때 Java 에서 Select 를 수행하면 INSERT, UPDATE, DELETE 된 데이터가
아닌 이전 상태읟 데이터가 조회된다.
간혹 이 상황에서 DBMS 가 CONNECTION에서 무한정 응답하지 않는 경우도 있다.
java는 DBMS가 응답하기를 기다리면서 무한정 기다리고 마치 프로젝트가 멈춘(Down) 상태가 되어버린다.

SQD 에서 INSERT, UPDATE, DELETE 를 수행한 다음에는 강제로 storage 에 commit 을 해줘야한다.
그래야만 Java 프로젝트에서 데이터를 조회할수 있다.
*/
COMMIT;

-- PK 칼럼을 기준으로 조건을 설정하여 조회하기
-- PK 칼럼을 기준으로 조회를 하면 데이터가 없거나, 1개만 조회된다.
SELECT buid, buname, butel, buaddr, bubirth, bujob
FROM tbl_buyer
WHERE buid = '0001';
...
UPDATE TBL_BUYER
SET BUNAME = '', BUTEL = '', BUADDR= '', BUBIRTH = '', BUJOB = ''
where BUID = '';
